...
- Make them available to PR submitters and reviewers for verification before
  pushing the PR and fixing issues.
...
- Cannot build some recipes that are very compute-intensive. Examples:
  ...
  - Such constraints could be handled by adding properties to recipes with such
    special needs.
...
- Some packages, have complex variants, and some variants (e.g. `~mpi`,
  `+serial`) may not be well-supported.
  - While known to users and maintainers of those packages such `notest` flags
    for variants may be helpful.
...
- For some packages, build dependency versions or variants not tested by the PR
  submitters may cause build failures
