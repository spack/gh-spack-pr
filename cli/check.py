...
def setup_spack_pr_extension() -> ExitCode:
    ...
    if not spawn("spack", ["--version"], show_command=False) == 0:
        print("Spack is not found, please clone and setup spack to use this tool.")
        return 1
...
def find_already_installed(specs_to_check: List[str]) -> Tuple[List[str], List[str]]:
    ...
    for spec in specs_to_check:
        err, stdout, _ = run(
            ["spack", "find", "--no-groups", "--show-full-compiler", "-v", "-I", spec]
        )
        if err == 0:
            print(stdout)
...
def spack_uninstall_packages(installed):
    ...
    for recipe in installed:
        ret, out, err = run(["spack", "uninstall", "-ya", "--dependents", recipe])
        print(out)
        if ret != 0:
            print(err or out)
            sys.exit(ret)
...
def parse_variant_infos(recipe: str) -> Tuple[ExitCode, dict, dict]:
    ...
    ret, stdout, stderr = run(["spack", "info", recipe])
    if ret:
        print(stderr or stdout)
        return ret, {}, {}
...
def add_extra_versions(specs, extraversions) -> None:
    ...
    for recipe, _ in recipes.items():
        ...
        exitcode, stdout, stderr = run(["spack", "info", *args, recipe])
        if exitcode:
...
def check_all_downloads(specs) -> ExitCode:
    ...
    for spec in specs:
        ...
        ret = spawn("spack", ["fetch", *fetch_flags, spec])
        if not ret:
            return ret
    return True
...
def spack_install(specs: Strs, args: argparse.Namespace) -> Tuple[Passes, Fails, Strs, Strs]:
    ...
    for spec in specs:
        ...
        with LogFile(install_log) as install_logfile:
            ret = spawn("spack", cmd, logfile=install_logfile)
...
def submit_request_for_spec(args, kind, report, spec):
    ...
    title = f"{hello} Could you check it?"
...
def add_compiler_to_specs(specs_to_check, args) -> List[str]:
    ...
    if args.compiler != "all":
        ...
        err, stdout, stderr = run(["spack", "compilers"])
        if err:
...
def get_maintainers(recipes: Strs) -> Strs:
    ...
    return getoutput(f"spack maintainers {' '.join(recipes)}").split()
...
def checkout_pr_by_number(pr_number: str) -> ExitCode:
    ...
    return spawn("spack", ["clean", "--stage"])
...
def run_commands_for_changed_files(args: argparse.Namespace) -> ExitCode:
    ...
    if args.command == "style":
        ...
        if which("isort") and which("black"):
            ...
            spawn("spack", ["style", *args.changed_files])
...
def abbreviated_spec_info(spec: str, spec_log_file: str) -> str:
    ...
    with LogFile(spec_log_file + ".spec") as spec_log:
        ...
        ret = spawn("spack", ["spec", spec], logfile=spec_log, output_filter=filter_spec_data)
        if ret:
...
def generate_build_results(installed: Strs, passed: Strs, fails: Fails, about_build_host) -> str:
    ...
    if installed or passed:
        ...
        logfile = BytesIO()
        err = spawn("spack", args + expected_specs, logfile=logfile)
